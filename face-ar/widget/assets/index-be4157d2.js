...
function instance($$self, $$props, $$invalidate) {
  ...
  component_subscribe($$self, percent, ($$value) => $$invalidate(7, $percent = $$value));
  ...
  validate_slots("FaceAnalysis", slots, []);
  ...
  Object.keys($$props).forEach((key) => {
    if (!~writable_props.indexOf(key) && key.slice(0, 2) !== "$$" && key !== "slot")
      console.warn(`<FaceAnalysis> was created with unknown prop '${key}'`);
  });
  ...
  return [$imageSource, screen, $routerPath, router, photo_handler, ended_handler];
}
class FaceAnalysis extends SvelteComponentDev {
  constructor(options) {
    super(options);
    init(this, options, instance, create_fragment, safe_not_equal, {});
    dispatch_dev("SvelteRegisterComponent", {
      component: this,
      tagName: "FaceAnalysis",
      options,
      id: create_fragment.name
    });
  }
}
const FaceAnalysis$1 = FaceAnalysis;
export {
  FaceAnalysis$1 as default
};
